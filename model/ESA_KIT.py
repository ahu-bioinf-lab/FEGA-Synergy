...
class MAB(nn.Module):
    def __init__(self, dim_Q, dim_K, dim_V, num_heads, dropout_p=0.0, xformers_or_torch_attn="torch"):  # 默认值改为 "torch"
        super(MAB, self).__init__()

        # 由于没有xformers，我们强制使用torch注意力
        # self.xformers_or_torch_attn = xformers_or_torch_attn
        self.xformers_or_torch_attn = "torch"
    ...
    def forward(self, Q, K, adj_mask=None):
        ...
        # 由于我们只使用 "torch" 模式，直接执行转置
        # if self.xformers_or_torch_attn in ["torch"]: # 这个if条件现在可以省略，因为我们已经强制为torch
        Q = Q.transpose(1, 2)
        K = K.transpose(1, 2)
        V = V.transpose(1, 2)
        ...
        # elif self.xformers_or_torch_attn in ["torch"]: # 这个if/elif现在可以省略，直接执行下面的代码
...
class SAB(nn.Module):
    def __init__(self, dim_in, dim_out, num_heads, dropout, xformers_or_torch_attn="torch"):  # 默认值改为 "torch"
        super(SAB, self).__init__()
        # 强制 MAB 使用 torch 注意力
        self.mab = MAB(dim_in, dim_in, dim_out, num_heads, dropout, xformers_or_torch_attn="torch")
...
class PMA(nn.Module):
    def __init__(self, dim, num_heads, num_seeds, dropout, xformers_or_torch_attn="torch"):  # 默认值改为 "torch"
        super(PMA, self).__init__()
        self.S = nn.Parameter(torch.Tensor(1, num_seeds, dim))
        nn.init.xavier_normal_(self.S)
        # 强制 MAB 使用 torch 注意力
        self.mab = MAB(dim, dim, dim, num_heads, dropout_p=dropout, xformers_or_torch_attn="torch")
